-- Complete bounded collection after domain references expire.
CREATE OR REPLACE FUNCTION public.proxyloom_tags_versioned() RETURNS trigger
...
BEGIN
    IF TG_OP='DELETE' AND current_setting('proxyloom.maintenance',true)=txid_current()::text
       AND current_user=pg_get_userbyid((SELECT proowner FROM pg_proc WHERE oid='public.proxyloom_cleanup(uuid,integer)'::regprocedure)) THEN RETURN OLD; END IF;
...
CREATE OR REPLACE FUNCTION public.proxyloom_cleanup(p_scope uuid,p_limit integer) RETURNS jsonb
...
BEGIN
 ...
 SELECT array_agg(id) INTO ids FROM (
  ...
  AND NOT EXISTS(SELECT 1 FROM public.compile_batches b WHERE b.base_publication_id=p.id AND (b.created_at>=publications_before OR EXISTS(SELECT 1 FROM public.jobs j WHERE j.batch_id=b.id AND j.finished_at IS NULL) OR (b.state='ready' AND NOT EXISTS(SELECT 1 FROM public.publications current_pub WHERE current_pub.batch_id=b.id))))
  AND NOT EXISTS(SELECT 1 FROM public.subscription_operations r WHERE r.operation_id=p.id)
  ORDER BY p.created_at,p.id LIMIT 1) picked;
 ...
 SELECT array_agg(id) INTO ids FROM (
  ...
  ORDER BY b.created_at,b.id LIMIT 1) picked;
 ...
 -- Retain the latest source preview metadata, even when its raw content expires.
 SELECT array_agg(id) INTO ids FROM (
  SELECT b.id FROM public.import_batches b JOIN public.jobs j ON j.id=b.job_id
  WHERE b.scope_id=p_scope AND b.created_at<publications_before AND j.finished_at IS NOT NULL
  AND NOT EXISTS(SELECT 1 FROM public.import_request_keys r WHERE r.batch_id=b.id)
  AND (b.source_id IS NULL OR EXISTS(SELECT 1 FROM public.import_batches newer WHERE newer.source_id=b.source_id AND (newer.created_at,newer.id)>(b.created_at,b.id)))
  ORDER BY b.created_at,b.id LIMIT 1) picked;
 DELETE FROM public.import_commit_items WHERE batch_id=ANY(ids);
 DELETE FROM public.import_commits WHERE batch_id=ANY(ids);
 DELETE FROM public.import_candidates WHERE batch_id=ANY(ids);
 DELETE FROM public.import_batches WHERE id=ANY(ids);
 GET DIAGNOSTICS n=ROW_COUNT; total:=total+n;
 DELETE FROM public.source_snapshots WHERE id IN (
  SELECT s.id FROM public.source_snapshots s WHERE s.scope_id=p_scope AND s.created_at<publications_before
  AND NOT EXISTS(SELECT 1 FROM public.import_batches b WHERE b.snapshot_id=s.id)
  AND EXISTS(SELECT 1 FROM public.source_snapshots newer WHERE newer.source_id=s.source_id AND newer.state=s.state AND (newer.created_at,newer.id)>(s.created_at,s.id))
  ORDER BY s.created_at,s.id LIMIT p_limit);
 GET DIAGNOSTICS n=ROW_COUNT; total:=total+n;

 -- Domain rows must release their references before their finished jobs expire.
 SELECT array_agg(id) INTO ids FROM (
  SELECT j.id FROM public.jobs j WHERE j.scope_id=p_scope AND j.finished_at<results_before
  AND NOT EXISTS(SELECT 1 FROM public.job_batches b WHERE b.id=j.batch_id)
  AND NOT EXISTS(SELECT 1 FROM public.test_jobs t WHERE t.job_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.import_batches b WHERE b.job_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.compile_batches b WHERE b.compile_job_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.compile_outputs o WHERE o.validation_job_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.idempotency_keys r WHERE r.operation_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.source_schedules s WHERE s.last_job_id=j.id)
  AND NOT EXISTS(SELECT 1 FROM public.quota_reservations q WHERE q.job_id=j.id AND q.settled_at IS NULL)
  ORDER BY j.finished_at,j.id LIMIT p_limit) picked;
 DELETE FROM public.job_events WHERE job_id=ANY(ids);
 DELETE FROM public.job_results WHERE job_id=ANY(ids);
 DELETE FROM public.job_payload_wrappings WHERE job_id=ANY(ids);
 DELETE FROM public.job_payloads WHERE job_id=ANY(ids);
 DELETE FROM public.quota_reservations WHERE job_id=ANY(ids);
 DELETE FROM public.jobs WHERE id=ANY(ids);
 GET DIAGNOSTICS n=ROW_COUNT; total:=total+n;
 DELETE FROM public.quota_buckets b WHERE b.scope_id=p_scope AND b.utc_day<results_before::date AND b.reserved_bytes=0
  AND NOT EXISTS(SELECT 1 FROM public.quota_reservations r WHERE r.scope_id=b.scope_id AND r.utc_day=b.utc_day);
 ...
 -- Soft deletion removes neither active credentials nor referenced history.
 -- Physically remove a tombstone only after all older revisions and every
 -- remaining reference are gone. Bound nodes/sources are retained explicitly.
 FOR rev IN SELECT h.id,h.head_revision FROM public.resources h WHERE h.scope_id=p_scope AND h.deleted_at<publications_before
  AND NOT EXISTS(SELECT 1 FROM public.resource_revisions r WHERE r.resource_id=h.id AND r.revision<>h.head_revision)
  AND NOT EXISTS(SELECT 1 FROM public.resource_refs r WHERE r.target_resource_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.publication_dependencies d WHERE d.resource_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.compile_dependencies d WHERE d.resource_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.compile_batches b WHERE b.profile_id=h.id OR (b.scope_id=p_scope AND NOT b.dependencies_registered))
  AND NOT EXISTS(SELECT 1 FROM public.idempotency_keys k WHERE k.resource_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.import_commit_items i WHERE i.resource_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.test_jobs t,jsonb_array_elements(t.dependencies) d WHERE t.scope_id=p_scope AND d->>'id'=h.id::text)
  AND NOT EXISTS(SELECT 1 FROM public.subscription_tokens t WHERE t.profile_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.node_bindings b WHERE b.node_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.source_items s WHERE s.source_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.source_snapshots s WHERE s.source_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.source_schedules s WHERE s.source_id=h.id)
  AND NOT EXISTS(SELECT 1 FROM public.import_batches b WHERE b.source_id=h.id)
  ORDER BY h.deleted_at,h.id LIMIT p_limit
 LOOP
  DELETE FROM public.resource_tags WHERE resource_id=rev.id;
  DELETE FROM public.resource_refs WHERE resource_id=rev.id;
  DELETE FROM public.resource_revision_wrappings WHERE resource_id=rev.id;
  DELETE FROM public.resource_revisions WHERE resource_id=rev.id;
  DELETE FROM public.resources WHERE id=rev.id;
  total:=total+1;
 END LOOP;
