...
| 编译骨架 | T-024 提供确定性 Prepare／标签／Plan；T-025～027 已对 Trojan TCP/TLS 输出三目标完整原生配置并经锁定 linux/amd64 内核正负检查。unverified 写入产物但不放行发布；原生检查通过不等于链路或客户端验证 | BE-A；T-028 再做真实链路，T-029 再补 P0 映射 |
