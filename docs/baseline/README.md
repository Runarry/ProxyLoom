...
任务状态唯一机器入口为 `docs/PLAN.tasks.json` 的 `tasks[].status`，阅读入口为 PLAN §12.3。本轮三项待评审，其余未开始；任务状态不得因拓扑排序自动提升。后续状态修改同时维护 PLAN §12.3 和证据，done 必须满足 PLAN §8.1。运行 `pwsh -NoProfile -File docs/baseline/check-plan.ps1` 检查清单、直接依赖 DAG 与估算；`-WriteTasks` 仅允许在机器清单不存在时初始生成，已有文件会被拒绝覆盖，以保护任务进度和证据历史。
