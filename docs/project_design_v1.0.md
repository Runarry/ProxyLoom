...
三个内核均生成受控 SOCKS5 入口，Go 通过统一 SOCKS5 拨号器建立 TCP 测试连接。HTTPS 探测验证目标证书、状态和响应特征，默认禁用跨站重定向、连接复用与压缩，以减少缓存和样本混杂。连通性使用三个完整的小响应样本，每个样本最多 64 KiB；超过该上限或未完整读取的响应不通过断言。探测目标使用自有或明确许可、无重定向且小响应的服务，禁止把公共服务当作无预算测速资源。
