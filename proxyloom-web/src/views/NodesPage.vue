...
<template>
  <main id="main" class="content"><div class="page-heading"><div><p class="eyebrow">工作空间 / 节点</p><h1>节点库</h1><p class="muted">整理连接，保留每一次配置修订。</p></div><div class="actions"><RouterLink class="button secondary" to="/imports">导入节点</RouterLink><RouterLink class="button" to="/nodes/new">＋ 创建节点</RouterLink></div></div>
    ...
    <section v-if="selected.size" class="panel batch-panel" aria-label="批量操作"><div class="section-heading"><strong>已选择 {{ selected.size }} / 200 个节点（跨页保留）</strong><button class="text-button" type="button" @click="selected.clear()">清空选择</button></div><form class="inline-form" @submit.prevent="batch"><label>批量操作<select v-model="batchOperation"><option value="enable">启用</option><option value="disable">停用</option><option value="add_tags">添加标签</option><option value="remove_tags">移除标签</option></select></label><label v-if="batchOperation === 'add_tags' || batchOperation === 'remove_tags'">操作标签<input v-model="batchTags" required placeholder="逗号分隔" /></label><button :disabled="batching || loading">{{ batching ? '正在处理…' : '应用到选中节点' }}</button></form><p v-if="batchOperation === 'disable'" class="hint">停用会立即撤销依赖该节点的发布访问。</p></section>
