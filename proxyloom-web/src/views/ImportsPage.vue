...
const selected = reactive(new Map<string, { candidate: Schema<'ImportCandidate'>; action: 'create' | 'update' }>())
...
const expired = computed(() => batch.value?.state === 'expired' || (!!batch.value?.expires_at && new Date(batch.value.expires_at).getTime() <= clockNow.value && batch.value.state !== 'committed'))
...
const stateLabels: Record<string, string> = { queued: '等待解析', parsing: '正在解析', ready: '等待确认', failed: '解析失败', committed: '已提交', expired: '已过期' }
...
function selectable(candidate: Schema<'ImportCandidate'>) { return !!candidate.node && candidate.state !== 'invalid' && !candidate.diagnostics.some(item => item.severity === 'error') }
...
function setSelection(candidate: Schema<'ImportCandidate'>, action: string) {
  if (action === 'skip') selected.delete(candidate.candidate_id)
  else if (selectable(candidate) && (selected.has(candidate.candidate_id) || selected.size < 5000)) {
    if (action === 'update' && (!candidate.existing_resource_id || !candidate.existing_revision)) return
    selected.set(candidate.candidate_id, { candidate, action: action as 'create' | 'update' })
...
function buildDecisions(): Schema<'ImportDecision'>[] {
  return [...selected.values()].map(({ candidate, action }) => action === 'update'
    ? { candidate_id: candidate.candidate_id, action, resource_id: candidate.existing_resource_id!, expected_revision: candidate.existing_revision! }
    : { candidate_id: candidate.candidate_id, action })
}
async function commit() {
  if (!batch.value || !ready.value || !selected.size || !commitConfirmed.value || conflict.value) return
  busy.value = true; error.value = null
  try {
    const body: Schema<'ImportCommitRequest'> = { decisions: buildDecisions() }
    ...
    if (!controller.signal.aborted) {
      error.value = failure
      if (failure instanceof APIError && failure.status === 412) { conflict.value = true; conflictReloaded.value = false }
    }
...
function acceptRefreshedPreview() {
  ...
  for (const [id, choice] of selected) {
    ...
    if (latest && choice.action === 'update') selected.set(id, { ...choice, candidate: { ...choice.candidate, existing_revision: latest.metadata.revision } })
  }
  latestTargets.value = []
...
async function compareConflict() {
  ...
  try {
    ...
    const ids = [...new Set([...selected.values()].filter(item => item.action === 'update').map(item => item.candidate.existing_resource_id!))]
...
function choiceForTarget(id: string) { return [...selected.values()].find(item => item.action === 'update' && item.candidate.existing_resource_id === id) }
...
<template>
  <main id="main" class="content"><div class="page-heading"><div><p class="eyebrow">工作空间 / 导入</p><h1>{{ id ? '检查导入预览' : '导入节点' }}</h1><p class="muted">读取分享链接，检查逐项诊断，再确认写入节点库。</p></div><RouterLink v-if="id" class="button secondary" to="/imports">新建导入</RouterLink></div>
    ...
    <template v-else-if="batch"><div class="summary-grid"><div class="panel"><span class="metric-label">预览状态</span><strong>{{ expired ? '已过期' : stateLabels[batch.state] }}</strong></div><div class="panel"><span class="metric-label">候选总数</span><strong>{{ batch.candidate_count }}</strong></div><div class="panel"><span class="metric-label">当前修订</span><strong>r{{ batch.revision }}</strong></div><div class="panel"><span class="metric-label">到期时间</span><strong class="small">{{ localTime(batch.expires_at) }}</strong></div></div><p class="hint resource-id">批次 {{ batch.batch_id }} · 任务 {{ batch.job_id }} · 创建于 {{ localTime(batch.created_at) }}</p>
      ...
      <section v-if="result || batch.state === 'committed'" class="panel result-panel" aria-live="polite"><span class="badge success">已原子提交</span><h2>导入已完成</h2><p v-if="result">本次创建 {{ result.items.filter(item => item.status === 'created').length }} 个节点，更新 {{ result.items.filter(item => item.status === 'updated').length }} 个节点。</p><p v-else>该批次已提交。刷新后仅恢复脱敏预览，可在节点库查看结果。</p><p class="hint">节点结构已保存，内核兼容性仍为未验证。</p><RouterLink class="button" to="/nodes">查看节点库</RouterLink></section>
