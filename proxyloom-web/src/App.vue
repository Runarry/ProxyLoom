...
<template>
  ...
  <div class="app-shell">
    <header class="site-header">
      ...
      <nav v-if="auth.user" aria-label="主要导航"><RouterLink to="/nodes" :class="{ active: route.path.startsWith('/nodes') }">节点</RouterLink><RouterLink to="/sources" :class="{ active: route.path.startsWith('/sources') }">来源</RouterLink><RouterLink to="/imports" :class="{ active: route.path.startsWith('/imports') }">导入</RouterLink><RouterLink to="/chains" :class="{ active: route.path.startsWith('/chains') }">链路</RouterLink><RouterLink to="/policy-groups" :class="{ active: route.path.startsWith('/policy-groups') }">策略组</RouterLink><RouterLink to="/routing" :class="{ active: route.path.startsWith('/routing') }">路由</RouterLink><RouterLink to="/rule-sets" :class="{ active: route.path.startsWith('/rule-sets') }">规则集</RouterLink><RouterLink to="/dns" :class="{ active: route.path.startsWith('/dns') }">DNS</RouterLink><RouterLink to="/client-presets" :class="{ active: route.path.startsWith('/client-presets') }">客户端预设</RouterLink></nav>
