...
function checkIR(name, seen = new Set()) {
  ...
  for (const dep of packages.get(name) ?? []) {
    assert.ok(!/^net\/http(?:\/|$)/.test(dep) && dep !== 'os/exec' && !database(dep) && !forbiddenLocal(dep), `IR boundary: ${name} imports ${dep}`);
...
console.log('PASS: IR project dependencies exclude HTTP/database/exec; Runner dependency graph excludes database/identity/server.');
