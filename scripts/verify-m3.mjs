// M3 acceptance owns its database, native test container and browser workspace.
...
assert.ok(['all', 'native', 'browser', 'recovery'].includes(mode), 'mode must be all, native, browser or recovery');
const id = randomUUID(), work = join(root, '.cache/m3', id), secretDir = join(work, 'secrets'), controlDir = join(work, 'control');
...
const fixtureNetwork = `proxyloom-m3-fixture-${id}`;
function safe(text) {
...
async function command(name, executable, args, options = {}) {
  ...
  writeFileSync(join(work, `${name}.txt`), assertSecretFree(safe(result.output), { path: `m3/${name}.txt`, secrets }), { mode: 0o600 });
...
try {
  const output = await command('database-start', process.execPath, ['scripts/acceptance-db.mjs', 'start', ...(['all', 'native'].includes(mode) ? ['--durable'] : [])]);
  ...
  if (mode === 'all') await command('postgres', 'go', ['test', '-mod=readonly', '-count=1', '-v', '-timeout=180s', '-run', '^TestPostgres(Publication|Subscription)', './internal/storage'], { env: { ...env, PROXYLOOM_REQUIRE_POSTGRES_TESTS: 'true' } });
  if (['all', 'recovery'].includes(mode)) {
    const binary = join(work, 'recovery.test');
    await command('build-recovery-suite', 'go', ['test', '-mod=readonly', '-c', '-o', binary, './internal/storage'], { env: { ...env, GOOS: 'linux', GOARCH: 'amd64', CGO_ENABLED: '0' } });
    chmodSync(binary, 0o755);
    report.recovery_binary_sha256 = createHash('sha256').update(readFileSync(binary)).digest('hex');
    const image = JSON.parse(readFileSync(join(root, 'deploy/tools.lock.json'), 'utf8')).images.postgres;
    await command('encrypted-backup-restore', 'docker', ['run', '--rm', '--name', `proxyloom-m3-recovery-${id}`, '--label', `io.proxyloom.m3=${id}`, '--network', network,
      '--user', '10001:10001', '--read-only', '--cap-drop=ALL', '--security-opt', 'no-new-privileges=true', '--pids-limit', '128', '--memory', '2g', '--cpus', '2',
      '--tmpfs', '/tmp:rw,nosuid,nodev,noexec,size=256m,mode=1777', '--mount', `type=bind,source=${binary},target=/suite/recovery.test,readonly`,
      ...['database_dsn', 'migration_dsn', 'admin_dsn'].flatMap(file => ['--mount', `type=bind,source=${join(secretDir, file)},target=/run/secrets/${file},readonly`]),
      '-e', 'GOMAXPROCS=2', '-e', 'PROXYLOOM_REQUIRE_POSTGRES_TESTS=true', '-e', 'PROXYLOOM_REQUIRE_RECOVERY_TESTS=true', '-e', 'PROXYLOOM_PG_TOOLS_DIR=/usr/lib/postgresql/17/bin',
      ...
      '--entrypoint', '/suite/recovery.test', image, '-test.v', '-test.run=^TestRecoveryPostgresBackupRestoreRealToolsAndOldAuthorization$', '-test.timeout=3m']);
  }
  if (mode !== 'recovery') {
    docker(['network', 'create', '--internal', '--subnet', '192.0.2.0/24', '--label', `io.proxyloom.m3=${id}`, fixtureNetwork]);
    await command('build-native-suite', 'go', ['test', '-mod=readonly', '-c', '-o', join(work, 'm3.test'), './internal/runnercontrol'], { env: { ...env, GOOS: 'linux', GOARCH: 'amd64', CGO_ENABLED: '0' } });
    chmodSync(join(work, 'm3.test'), 0o755);
    report.test_binary_sha256 = createHash('sha256').update(readFileSync(join(work, 'm3.test'))).digest('hex');
    const image = JSON.parse(readFileSync(join(root, 'deploy/tools.lock.json'), 'utf8')).images.runtime;
    const dockerArgs = (name, extra = []) => ['run', '--rm', '--name', name, '--label', `io.proxyloom.m3=${id}`, '--network', `name=${fixtureNetwork},ip=192.0.2.2`, '--network', network, '--user', '10002:10002', '--read-only', '--cap-drop=ALL', '--security-opt', 'no-new-privileges=true', '--pids-limit', '128', '--memory', '2g', '--cpus', '2', '--tmpfs', '/tmp:rw,nosuid,nodev,noexec,size=256m,mode=1777', '--mount', `type=bind,source=${join(work, 'm3.test')},target=/suite/m3.test,readonly`, '--mount', `type=bind,source=${join(root, '.cache/cores')},target=/cores,readonly`, ...['database_dsn', 'migration_dsn', 'admin_dsn'].flatMap(file => ['--mount', `type=bind,source=${join(secretDir, file)},target=/run/secrets/${file},readonly`]), '-e', 'GOMAXPROCS=2', '-e', 'PROXYLOOM_REQUIRE_RUNNER_TESTS=true', '-e', 'PROXYLOOM_RUNNER_REAL_CORES=/cores', '-e', 'PROXYLOOM_TEST_DATABASE_DSN_FILE=/run/secrets/database_dsn', '-e', 'PROXYLOOM_TEST_MIGRATION_DSN_FILE=/run/secrets/migration_dsn', '-e', 'PROXYLOOM_TEST_ADMIN_DSN_FILE=/run/secrets/admin_dsn', ...extra, image, '/suite/m3.test', '-test.v', '-test.run=^TestPostgresM3RealNetwork$', '-test.timeout=9m'];
    if (['all', 'native'].includes(mode)) {
      chmodSync(controlDir, 0o777);
      let complete = false, restarted = false;
      const native = command('native-network', 'docker', dockerArgs(`proxyloom-m3-native-${id}`, ['--mount', `type=bind,source=${controlDir},target=/control`, '-e', 'PROXYLOOM_M3_FAULT_CONTROL_DIRECTORY=/control'])).finally(() => { complete = true; });
      const inject = (async () => {
        while (!complete) {
          if (!restarted && existsSync(join(controlDir, 'restart-database'))) {
            await command('restart-database-during-download', 'docker', ['restart', network]);
            let available = false;
            for (let attempt = 0; attempt < 100; attempt++) {
              const probe = spawnSync('docker', ['exec', network, 'pg_isready', '-q', '-U', 'postgres', '-d', 'postgres'], { windowsHide: true, timeout: 5000 });
              if (probe.status === 0) { available = true; break; }
              await new Promise(done => setTimeout(done, 100));
            }
            assert.ok(available, 'database_did_not_recover');
            writeFileSync(join(controlDir, 'database-restarted'), 'restarted'); restarted = true;
          }
          await new Promise(done => setTimeout(done, 100));
        }
      })();
      await Promise.all([native, inject]);
      assert.ok(restarted, 'database_restart_fault_not_exercised');
    }
    if (['all', 'browser'].includes(mode)) {
      const password = `M3-${randomBytes(24).toString('base64url')}!`; secrets.push(password);
      writeFileSync(join(secretDir, 'browser_password'), password, { mode: 0o444 });
      const port = await freePort(), baseURL = `http://127.0.0.1:${port}`, name = `proxyloom-m3-browser-${id}`;
      ...
      try {
        await command('browser', process.execPath, ['node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/tests.spec.ts', '--config', 'playwright.config.ts', '--output', join(work, 'browser-results')], { cwd: join(root, 'proxyloom-web'), env: { ...env, PROXYLOOM_E2E_BASE_URL: baseURL, PROXYLOOM_M3_DB_CONTAINER: network, PROXYLOOM_E2E_USERNAME: 'm3-admin', PROXYLOOM_E2E_PASSWORD: password, PROXYLOOM_E2E_BROWSER: process.env.PROXYLOOM_E2E_BROWSER ?? 'msedge', PROXYLOOM_M3_SEED: readFileSync(join(controlDir, 'ready'), 'utf8').trim() } });
      } finally {
  ...
  for (const name of [`proxyloom-m3-native-${id}`, `proxyloom-m3-browser-${id}`, `proxyloom-m3-recovery-${id}`]) {
    const found = spawnSync('docker', ['inspect', '--format', '{{index .Config.Labels "io.proxyloom.m3"}}', name], { encoding: 'utf8', windowsHide: true });
    if (found.status === 0 && found.stdout.trim() === id) { docker(['rm', '--force', name]); report.cleanup.push({ name, result: 'pass' }); }
  }
  const ownedFixture = spawnSync('docker', ['network', 'inspect', '--format', '{{index .Labels "io.proxyloom.m3"}}', fixtureNetwork], { encoding: 'utf8', windowsHide: true });
  if (ownedFixture.status === 0 && ownedFixture.stdout.trim() === id) { docker(['network', 'rm', fixtureNetwork]); report.cleanup.push({ name: 'fixture-network', result: 'pass' }); }
