...
const fixtures = join(root, 'fixtures', 'runner', 'validate');
...
function fail(message) {
  throw new Error(`Core exec: ${message}`);
}
...
function familySpec(family) {
  if (family === 'xray') {
    return { valid: 'xray.valid.json', invalid: 'xray.invalid.json', check: '/work/core run -test -c /work/config.json', run: '/work/core run -c /work/config.json', config: 'xray.valid.json' };
  }
  if (family === 'sing-box') {
    return { valid: 'singbox.valid.json', invalid: 'singbox.invalid.json', check: '/work/core check -c /work/config.json', run: '/work/core run -c /work/config.json', config: 'singbox.valid.json' };
  }
  if (family === 'mihomo') {
    return { valid: 'mihomo.valid.yaml', invalid: 'mihomo.invalid.yaml', check: '/work/core -t -d /work -f /work/config.yaml', run: '/work/core -d /work -f /work/config.yaml', config: 'mihomo.valid.yaml' };
  }
...
for (const build of builds) {
  ...
  const configName = build.family === 'mihomo' ? 'config.yaml' : 'config.json';
  const common = [
    'run', '--rm', '--network', 'none', '--read-only',
    '--tmpfs', '/work:uid=10002,mode=0700,exec',
    '--user', '10002:10002',
    '-v', `${toPosix(coreDir)}:/cores-bin:ro`,
    '-v', `${toPosix(fixtures)}:/fixtures:ro`,
    image,
    '/bin/sh', '-c',
  ];
  const validCopy = `cp /fixtures/${spec.valid} /work/${configName}`;
  const invalidCopy = `cp /fixtures/${spec.invalid} /work/${configName}`;
  const checkValid = docker([...common, script(build.binary_name, validCopy, spec.check, false)]);
  if (checkValid.status !== 0) {
    fail(`${build.family} valid check failed: ${(checkValid.stderr || checkValid.stdout || '').slice(0, 400)}`);
  }
  const checkInvalid = docker([...common, script(build.binary_name, invalidCopy, spec.check, false)]);
  if (checkInvalid.status === 0) {
    fail(`${build.family} invalid check exited 0`);
  }
  ...
  if (start.status !== 0 || !(start.stdout || '').includes('START_STOPPED')) {
    fail(`${build.family} start/stop failed: ${(start.stderr || start.stdout || '').slice(0, 400)}`);
  }
  report.push({
    family: build.family,
    id: build.id,
...
if (report.some((row) => /secret|password|EXAMPLE_ONLY_/i.test(JSON.stringify(row)))) {
  fail('report contained secrets');
}
process.stdout.write(`${JSON.stringify({ image, network: 'none', results: report }, null, 2)}\n`);
process.stdout.write('PASS: locked linux/amd64 cores accepted valid configs, rejected invalid configs, and reaped loopback start.\n');
